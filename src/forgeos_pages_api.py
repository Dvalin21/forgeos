...
# ──────────────────────────────────────────────────────────────
# FORGEDB EXTENSIONS  — create / import / network info
# ──────────────────────────────────────────────────────────────
# These supplement the existing /api/filedb/* (proxy) endpoints with
# operations that need to run *here* on the NAS rather than in the daemon.

FILEDB_WATCH_ROOT = Path(os.environ.get("FORGEDB_WATCH_ROOT", "/srv/nas"))
FILEDB_DAEMON_URL = os.environ.get("FILEDB_DAEMON_URL", "http://127.0.0.1:12010")


def _db_dir_safe(path: str) -> Path:
    """Resolve a target dir, refusing anything outside the watch root."""
    if not path:
        raise HTTPException(400, "Directory required")
    p = Path(path)
    if not p.is_absolute():
        p = FILEDB_WATCH_ROOT / p
    rp = p.resolve()
    root = FILEDB_WATCH_ROOT.resolve()
    if rp != root and root not in rp.parents:
        raise HTTPException(403, f"Path must be under {root}")
    return rp


def _ping_daemon(path: str = "/health") -> tuple[bool, str]:
    """Quick best-effort check that the daemon is up."""
    import urllib.request, urllib.error
    try:
        with urllib.request.urlopen(FILEDB_DAEMON_URL + path, timeout=2) as r:
            return r.status == 200, ""
    except Exception as e:
        return False, str(e)


def _rescan_daemon() -> None:
    """Best-effort: tell the daemon to re-discover databases.
    Falls back silently if the daemon doesn't expose /api/rescan yet —
    the next periodic scan will pick changes up anyway."""
    import urllib.request
    try:
        req = urllib.request.Request(FILEDB_DAEMON_URL + "/api/rescan",
                                     method="POST", data=b"")
...
@router.get("/api/filedb/network")
async def filedb_network(user=Depends(verify_token)):
    """How clients on the LAN should connect to this ForgeDB-coordinated share."""
    import socket as _s
    host = _s.gethostname()
    try:
        ...
        ip = "127.0.0.1"
    # mDNS service file the daemon installs (see forgeos-filedb.py).
    avahi = Path("/etc/avahi/services/forgeos-filedb.service")
    mdns_published = avahi.is_file()
    # Pull the share root from samba; otherwise default to /srv/nas.
    return {
        "hostname": host,
        "lan_ip": ip,
        "mdns_published": mdns_published,
        "mdns_service_types": ["_edb-server._tcp", "_forgefiledb._tcp"],
        "edb_port": 12010,
        "share_root": str(FILEDB_WATCH_ROOT),
        "smb_url_pattern": f"smb://{host}/<share-name>",
        "smb_url_ip": f"\\\\{ip}\\<share-name>",
        "discovery_hint":
            "Apps that support EDB-style discovery (ElevateDB, Atrex) "
            "will see this NAS automatically on the LAN. For apps without "
            "discovery, point them at the SMB path above.",
    }


@router.post("/api/filedb/dir/prepare")
async def filedb_dir_prepare(body: dict, user=Depends(verify_token)):
    """Create an empty, properly-permissioned directory under the watch
    root, ready for a database app (ElevateDB/Atrex/Access/NexusDB) to
    write its files into. The directory becomes a ForgeDB-tracked dir
    once a recognized DB file appears in it.

    Body: { name, owner?, group?, mode? (octal str) }
    """
    _admin(user)
    name = re.sub(r"[^A-Za-z0-9 ._-]", "_", body.get("name", "")).strip()
    if not name:
        raise HTTPException(400, "Name required")
    parent = _db_dir_safe(body.get("parent", str(FILEDB_WATCH_ROOT)))
    target = parent / name
    if target.exists():
        raise HTTPException(409, f"{target} already exists")
    target.mkdir(parents=True, exist_ok=False)
    # POSIX mode (default 770: rwx for owner+group, none for others).
    mode_raw = str(body.get("mode", "770")).strip()
    if re.fullmatch(r"[0-7]{3,4}", mode_raw):
        os.chmod(target, int(mode_raw, 8))
    # chown if owner/group given. We don't error on missing names —
    # the directory is still created and usable.
    own = re.sub(r"[^A-Za-z0-9._-]", "", str(body.get("owner", "")))
    grp_ = re.sub(r"[^A-Za-z0-9._-]", "", str(body.get("group", "")))
    if own or grp_:
        _run(["chown", (own + (":" + grp_ if grp_ else "")) or (":" + grp_),
              str(target)], timeout=5)
    _rescan_daemon()
    _audit(user["sub"], "filedb.dir.prepare", "success",
           f"{target} (mode={mode_raw}, owner={own or '-'}, group={grp_ or '-'})")
    return {
        "ok": True,
        "path": str(target),
        "next_step": "Point your database application at this directory. "
                     "ForgeDB will start tracking it as soon as a DB file appears.",
    }


@router.post("/api/filedb/sqlite/create")
async def filedb_sqlite_create(body: dict, user=Depends(verify_token)):
    """Create a real, valid SQLite database. Optionally run an initial
    CREATE-TABLE script (validated by sqlite3 itself before persisting).

    Body: { dir, filename, schema? (SQL string) }
    """
    _admin(user)
    import sqlite3 as _sql
    target_dir = _db_dir_safe(body.get("dir", ""))
    if not target_dir.is_dir():
        raise HTTPException(400, "Directory does not exist")
    fname = re.sub(r"[^A-Za-z0-9._-]", "_", body.get("filename", "")).strip()
    if not fname:
    ...
    if dst.exists():
        raise HTTPException(409, f"{dst} already exists")
    schema = body.get("schema") or ""
    if schema and not isinstance(schema, str):
        raise HTTPException(400, "schema must be a SQL string")
    if schema and len(schema) > 100_000:
        raise HTTPException(400, "schema too large (limit 100 KB)")
    # Build in a temp path; only rename into place if everything succeeds.
    tmp = dst.with_suffix(dst.suffix + ".forgeos.new")
    try:
        conn = _sql.connect(str(tmp))
        try:
            if schema.strip():
        ...
        if tmp.exists():
            tmp.unlink(missing_ok=True)
        raise HTTPException(500, f"Could not create database: {e}")
    _rescan_daemon()
    _audit(user["sub"], "filedb.sqlite.create", "success",
           f"{dst} (schema={'yes' if schema.strip() else 'no'})")
    return {"ok": True, "path": str(dst), "size": dst.stat().st_size}


@router.post("/api/filedb/dbase/create")
async def filedb_dbase_create(body: dict, user=Depends(verify_token)):
    """Create a real, valid dBase III (.dbf) file from a column schema.
    Uses the `dbf` library (pip install dbf) for correctness.

    Body: { dir, filename, columns: [{name, type, length, decimal?}] }
      type ∈ { 'C' (char), 'N' (numeric), 'D' (date YYYYMMDD), 'L' (logical) }
    """
    _admin(user)
    try:
        import dbf as _dbf                       # ships with pip install dbf
    except ImportError:
        raise HTTPException(
            501,
            "dBase support requires the 'dbf' Python package. "
    ...
    if len(cols) > 254:                          # dBase hard limit
        raise HTTPException(400, "dBase supports at most 254 columns")

    # Validate + build the schema string in `dbf`'s format
    # ("NAME C(30); AGE N(3,0); ACTIVE L; DOB D")
    seen = set()
    schema_parts = []
    for raw in cols:
        if not isinstance(raw, dict):
            raise HTTPException(400, "Each column must be an object")
        cname = re.sub(r"[^A-Za-z0-9_]", "", str(raw.get("name", "")))[:10].upper()
        ctype = str(raw.get("type", "C")).upper()
        if not cname:
            raise HTTPException(400, "Column name required (A–Z, 0–9, _ only)")
        if cname in seen:
            raise HTTPException(400, f"Duplicate column name: {cname}")
        if ctype not in ("C", "N", "D", "L"):
            raise HTTPException(400, f"Column {cname}: type must be C/N/D/L")
        try:
            length = int(raw.get("length", 0))
        except (TypeError, ValueError):
            raise HTTPException(400, f"Column {cname}: invalid length")
        decimal = int(raw.get("decimal", 0) or 0)
        if ctype == "C":
            if not 1 <= length <= 254:
                raise HTTPException(400, f"Column {cname}: C length 1..254")
            schema_parts.append(f"{cname} C({length})")
        elif ctype == "N":
            if not 1 <= length <= 20:
                raise HTTPException(400, f"Column {cname}: N length 1..20")
            if decimal < 0 or decimal >= length:
                raise HTTPException(400,
                                    f"Column {cname}: invalid decimal count")
            schema_parts.append(f"{cname} N({length},{decimal})")
        elif ctype == "D":
            schema_parts.append(f"{cname} D")
        elif ctype == "L":
            schema_parts.append(f"{cname} L")
        seen.add(cname)
    schema = "; ".join(schema_parts)

    try:
        t = _dbf.Table(str(dst), schema, dbf_type="db3")
        t.open(_dbf.READ_WRITE)
        t.close()
    except Exception as e:
        if dst.exists():
            dst.unlink(missing_ok=True)
        raise HTTPException(400, f"dBase creation failed: {e}")

    _rescan_daemon()
    _audit(user["sub"], "filedb.dbase.create", "success",
           f"{dst} ({len(cols)} cols)")
    return {"ok": True, "path": str(dst), "columns": len(cols),
            "size": dst.stat().st_size}


@router.post("/api/filedb/register")
async def filedb_register(body: dict, user=Depends(verify_token)):
    """Tell the daemon to start tracking an existing directory of DB files.
    Falls through to a daemon rescan if the daemon doesn't expose an
    explicit /register endpoint."""
    _admin(user)
    target = _db_dir_safe(body.get("dir", ""))
    if not target.is_dir():
        raise HTTPException(404, "Directory not found")
    # Best-effort: ask the daemon to register, otherwise rescan.
    import urllib.request, urllib.error, json as _json
    payload = _json.dumps({"dir": str(target)}).encode()
    registered = False
    try:
        req = urllib.request.Request(
            FILEDB_DAEMON_URL + "/api/register",
        ...
        # Daemon answered but didn't like the request — surface it.
        if e.code == 404:
            registered = False                             # no endpoint, fall through
        else:
            raise HTTPException(e.code, e.read().decode("utf-8", "replace"))
    except Exception:
        registered = False
    if not registered:
        _rescan_daemon()
    _audit(user["sub"], "filedb.register", "success", str(target))
    return {"ok": True, "path": str(target),
            "method": "register" if registered else "rescan"}


@router.post("/api/filedb/rescan")
async def filedb_rescan(user=Depends(verify_token)):
    """Force the daemon to rediscover databases now."""
    _admin(user)
    _rescan_daemon()
    _audit(user["sub"], "filedb.rescan", "success", "")
    return {"ok": True}
...
# ──────────────────────────────────────────────────────────────
# FIREWALL  — IPv4 + IPv6, fronted by ufw (shown as "Firewall")
# ──────────────────────────────────────────────────────────────
# ufw manages both iptables and ip6tables and handles persistence,
# so we keep it as the engine but never surface the name "ufw".

def _ufw(args: list[str], timeout: int = 15) -> subprocess.CompletedProcess:
    return _run(["ufw"] + args, timeout=timeout)


def _parse_ufw_rules(text: str) -> list[dict]:
    """Parse `ufw status numbered` into structured rules (v4 & v6)."""
    rules = []
    for line in text.splitlines():
        ...
        # ufw columns: TO  ACTION  FROM  (ACTION may include 'IN'/'OUT')
        parts = re.split(r"\s{2,}", body.strip())
        to = parts[0] if parts else ""
        action = parts[1] if len(parts) > 1 else ""
        frm = parts[2] if len(parts) > 2 else "Anywhere"
        is_v6 = "(v6)" in to or "(v6)" in frm
        rules.append({
            "num": num,
            "to": to.replace("(v6)", "").strip(),
            "action": action.strip(),
            "from": frm.replace("(v6)", "").strip() or "Anywhere",
...
async def firewall_status(user=Depends(verify_token)):
    st = _ufw(["status", "verbose"]).stdout
    numbered = _ufw(["status", "numbered"]).stdout
    active = bool(re.search(r"Status:\s*active", st, re.I))
    default_in = (re.search(r"Default:\s*(\w+)\s*\(incoming\)", st) or [None, "deny"])[1]
    default_out = (re.search(r"\(incoming\),\s*(\w+)\s*\(outgoing\)", st) or [None, "allow"])[1]
    return {
        "active": active,
        "default_incoming": default_in,
        "default_outgoing": default_out,
        "rules": _parse_ufw_rules(numbered),
    }


# Friendly service presets for the "caveman-simple" picker.
FIREWALL_SERVICES = [
...
async def firewall_toggle(body: dict, user=Depends(verify_token)):
    ...
    r = _ufw(["--force", "enable" if on else "disable"], timeout=20)
    if r.returncode != 0:
        raise HTTPException(400, r.stderr.strip() or "firewall toggle failed")
...
async def firewall_add_rule(body: dict, user=Depends(verify_token)):
    """Add a rule. Body:
        ...
        port:    '443/tcp' | '53' | '1000:2000/udp'
        from:    'any' | CIDR | IP
        family:  both | ipv4 | ipv6   (default both)

    Family handling:
      • from == 'any':
          - both  → one `ufw <action> <port>` call (ufw applies v4+v6 when
                    IPV6=yes in /etc/default/ufw).
          - ipv4  → `ufw <action> from 0.0.0.0/0 to any port <p> [proto]`
          - ipv6  → `ufw <action> from ::/0      to any port <p> [proto]`
      • from is a specific address: family is dictated by that address; we
        verify the requested family matches and reject a mismatch rather than
        silently creating the wrong rule.
    """
    _admin(user)
    action = body.get("action", "allow")
    if action not in ("allow", "deny", "reject"):
        raise HTTPException(400, "action must be allow, deny or reject")
    port = str(body.get("port", "")).strip()
    if not re.fullmatch(r"\d{1,5}(:\d{1,5})?(/(tcp|udp))?", port):
        raise HTTPException(400, "Invalid port spec (e.g. 443/tcp, 53, 1000:2000/udp)")
    src = str(body.get("from", "any")).strip() or "any"
    if src != "any" and not re.fullmatch(r"[0-9a-fA-F:.\/]+", src):
        raise HTTPException(400, "Invalid source address")
    family = body.get("family", "both")
    if family not in ("both", "ipv4", "ipv6"):
        raise HTTPException(400, "family must be both, ipv4 or ipv6")

    portnum = port.split("/")[0]
    proto = port.split("/")[1] if "/" in port else None

    def addr_family(a: str) -> str:
        return "ipv6" if ":" in a else "ipv4"

    # Decide which concrete source(s) to apply the rule to.
    if src != "any":
        af = addr_family(src)
        if family != "both" and family != af:
            raise HTTPException(
                400, f"Source {src} is {af}, but family was set to {family}")
        sources = [src]                       # specific address → single family
    else:
        if family == "both":
            sources = [None]                  # bare rule, ufw covers v4+v6
        else:
            sources = ["0.0.0.0/0" if family == "ipv4" else "::/0"]

    applied = []
    for s in sources:
        args = ["ufw", action]
        if s is None:
            args += [port]                    # e.g. ufw allow 443/tcp
        else:
            args += ["from", s, "to", "any", "port", portnum]
            if proto:
                args += ["proto", proto]
        r = _run(args, timeout=15)
        if r.returncode != 0:
            raise HTTPException(400, r.stderr.strip() or "rule add failed")
        applied.append(r.stdout.strip())

    _audit(user["sub"], "firewall.rule.add", "success",
           f"{action} {port} from {src} (family={family})")
    return {"ok": True, "family": family, "message": "\n".join(applied)}


@router.delete("/api/firewall/rule/{num}")
async def firewall_delete_rule(num: int, user=Depends(verify_token)):
    _admin(user)
    r = _run(["ufw", "--force", "delete", str(num)], timeout=15)
    if r.returncode != 0:
        raise HTTPException(400, r.stderr.strip() or "rule delete failed")
    _audit(user["sub"], "firewall.rule.delete", "success", f"rule #{num}")
    return {"ok": True, "message": r.stdout.strip()}
...
async def firewall_defaults(body: dict, user=Depends(verify_token)):
    ...
    for direction in ("incoming", "outgoing"):
        ...
        if pol in ("allow", "deny", "reject"):
            r = _ufw(["default", pol, direction])
