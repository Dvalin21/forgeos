...
ReadWritePaths=/etc/forgeos /var/log/forgeos /opt/forgeos /srv -/etc/samba -/etc/nginx -/etc/fail2ban -/etc/letsencrypt -/var/log/letsencrypt -/var/lib/letsencrypt -/etc/exports.d -/etc/wireguard -/etc/rear -/etc/ufw -/etc/snapper -/etc/apt/apt.conf.d -/run
...
LOGO
    ...
    [[ "${FEATURE_STORAGE:-}"    == "true" ]] && echo "  ✓ ForgeRAID Storage (mdadm+btrfs, hot-swap, SMART)"
    [[ "${FEATURE_DOCKER:-}"     == "true" ]] && echo "  ✓ Docker CE + Incus"
    [[ "${FEATURE_SECURITY:-}"   == "true" ]] && echo "  ✓ Security (UFW, Fail2ban, CrowdSec, AppArmor)"
    [[ "${FEATURE_MONITORING:-}" == "true" ]] && echo "  ✓ Monitoring (Prometheus, Grafana, Alertmanager)"
    [[ "${FEATURE_FILESHARE:-}"  == "true" ]] && echo "  ✓ File Sharing (Samba, NFS, FTPS, WebDAV)"
    [[ "${FEATURE_VPN:-}"        == "true" ]] && echo "  ✓ VPN (WireGuard + Netbird)"
    [[ "${FEATURE_PROXY:-}"      == "true" ]] && echo "  ✓ Reverse Proxy (nginx + Let's Encrypt)"
    [[ "${FEATURE_MAIL:-}"       == "true" ]] && echo "  ✓ Mail Server (Postfix+Dovecot+Rspamd+SOGo)"
    [[ "${FEATURE_BACKUP:-}"     == "true" ]] && echo "  ✓ Backup (Restic+Rclone+Snapper)"
    [[ "${FEATURE_CLOUD:-}"      == "true" ]] && echo "  ✓ Cloud Storage (RustFS S3 + rclone crypt)"
    [[ "${FEATURE_HIPAA:-}"      == "true" ]] && echo "  ✓ HIPAA Compliance Mode"
    echo ""
    echo "  ── Subdomains ─────────────────────────────────────"
    echo "    grafana.${d}   — Metrics dashboard"
    echo "    photos.${d}    — Immich photo library"
    echo "    mail.${d}      — SOGo webmail"
    echo "    console.s3.${d} — RustFS S3 console"
